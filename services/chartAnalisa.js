...
const { executablePath } = require("@sparticuz/chromium"); // ✅ named import
...
async function generateCandlestickChart(symbol = "BBCA.JK") {
  ...
  const browser = await puppeteer.launch({
    ...
    executablePath: await executablePath(), // ✅ TANPA ARGUMEN
    headless: true,
  ...
  if (ohlc.length === 0) throw new Error("Tidak ada data valid");

  const closes = ohlc.map((d) => d.close);
  const volumes = ohlc.map((d) => d.volume || 0);

  // Hitung indikator
  const ma5 = technicalIndicators.SMA.calculate({ period: 5, values: closes });
